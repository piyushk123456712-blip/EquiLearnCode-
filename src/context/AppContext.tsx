...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [videoOverrides, setVideoOverrides] = useState<Record<string, VideoOverride>>(() => {
    const saved = localStorage.getItem('videoOverrides');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('videoOverrides', JSON.stringify(videoOverrides));
  }, [videoOverrides]);
  ...
  const updateLessonVideo = async (lessonId: string, data: { youtubeUrlEn?: string; youtubeUrlHi?: string; mp4File?: File }) => {
    ...
    setVideoOverrides(prev => {
      ...
      const updated: Record<string, VideoOverride> = {
        ...prev,
        [lessonId]: {
  ...
  const allCourses = useMemo(() => {
    const coursesCopy = JSON.parse(JSON.stringify(defaultCourses)); // deep clone
    
    // Apply video overrides to default courses
